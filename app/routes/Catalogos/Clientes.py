...
from app.services.Catalogos.Clientes.clientes_service import verClienteID, verClientes, actCliente, getPaisEstado
...
getPaisEstado_bp = Blueprint('getPaisEstado', __name__)
...
@getPaisEstado_bp.route('/Catalogos/Clientes/getPaisEstado', methods=['POST'])
@jwt_required()
def getPaisEstado_route():
    data = request.json
    if data is None:
        return jsonify({"error": "Faltan datos requeridos"}), 400
    response = getPaisEstado(data)
    return response